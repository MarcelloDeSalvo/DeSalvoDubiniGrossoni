<template>
    <article class="rounded-xl bg-white p-3 shadow-lg hover:shadow-xl hover:transform hover:scale-105 duration-300 ">
      <a href="#">
        <div class="mt-1 p-2">
          <h2 class="text-slate-700">Discount: {{ discountID }}</h2>
          ...
          <p v-for="station in applied_stations" class="mt-1 text-sm text-slate-400"> - ID: {{ station.station_id }}</p>
          
          <div class="mt-3 flex items-end justify-between">
            <p>
              <span v-text="discount_amount" class="text-lg font-bold text-blue-500"></span>
            </p>
...
export default {
    props: {
        discountID: {   //contains address and civic number
            type: String,
            required: true,
        },
        start_date: {
